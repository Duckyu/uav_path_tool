...
def rotate3d(x, y, z, base_x, base_y, base_z, angle):
    xx = x
    yy = np.cos(angle) * (y - base_y) - np.sin(angle) * (z - base_z) + base_y
    zz = np.sin(angle) * (y - base_y) + np.cos(angle) * (z - base_z) + base_z
    return xx, yy, zz
...
def main():
    ...
    uav_setup_l = []
    isSin = [False, False, False, False, False]
    for line in rdr:
        if line[0] == path_num:
            if line[1] == 'sin':
                isSin[int(line[2])-1] = True
    ...
    ###########################################
    for i, yz in enumerate(use_yz):
        if yz and amplitude[i] > min(height[i]) - 3:
            print('Ground Collision !!!!!!!')
            print('Retry index {}'.format(i+1))
            # exit()
    for l in range(uav_size):
        if not any(isSin):
            ...
            print("UAV{} is not trigonometric path".format(l+1))
            continue
        ...
        for j in range(iter):
            if j > 0:
                continue
            for i in range(resolution * iter):
                angle = 2*np.pi/float(resolution)*i
                x_ = origin_x[l] + angle
                y_ = origin_y[l] + amplitude[l] * np.sin(angle/period[l])
                z_ += delta_z
                if not use_yz[l]:
                    x, y = rotate(x_, y_, origin_x[l], origin_y[l], origin_yaw[l])
                    z = z_
                else:
                    x, y, z = rotate3d(x_, y_, z_, origin_x[l], origin_y[l], \
                                height[l][0], np.pi/2)
                if not no_turn[l]:
